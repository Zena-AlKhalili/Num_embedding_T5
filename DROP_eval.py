...
def evaluate_prediction_file(
    prediction_path: str, gold_path: str, output_path: Optional[str] = None, args = None
) -> Tuple[float, float]:
    """
    ...
    """
    if prediction_path.endswith('.json'):
        predicted_answers = json.load(open(prediction_path, encoding="utf-8"))
    elif prediction_path.endswith('.jsonl'):
        with jsonlines.open(prediction_path, 'r') as reader:
            predicted_answers = {d['query_id']:d[args.answer_key] for d in reader.iter()}
...
#         required=False,
#         default='answer',
#         help="in case of a .jsonl prediction file, the key corresponding to the prediction string.",
#     )
    
#     args = parser.parse_args()
#     evaluate_prediction_file(args.prediction_path, args.gold_path, args.output_path, args)
